...
class TestTriggerNodeRegistry:
    ...
    def test_16_builtin_nodes_plus_3_trigger_plus_4_specialist_plus_review_gate_equals_24(self):
        ...
        registry = create_default_registry()
        types = registry.list_types()
        assert len(types) == 33  # +3 for EvidenceSearch, ClaimVerifier, ContradictionScan, VerificationSummary  # 16 original + 3 trigger + 4 specialist + 1 review_gate + 4 new specialist (planner, auditor, compliance, code)
