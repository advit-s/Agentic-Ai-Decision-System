...
from decision_system.context.store import save_context, load_context
...
def test_select_relevant_ontology_concepts_matches():
    ...
    from decision_system.orchestration.problem_analyzer import ProblemAnalysis
    from decision_system.ontology.models import OntologyMap, OntologyConcept
    analysis = ProblemAnalysis(
        question="Where are we losing money?",
        decision_type="financial",
        required_ontology_concepts=["revenue", "expense", "profit_margin"],
    )
    omap = OntologyMap(concepts=[
        ...
        OntologyConcept(concept_id="profit_margin", name="Profit Margin", concept_type="metric"),
...
def test_select_relevant_insights_financial_question():
    ...
    from decision_system.orchestration.problem_analyzer import ProblemAnalysis
    from decision_system.insights.models import InsightStore, Insight
    analysis = ProblemAnalysis(
        question="Where are we losing money?",
        decision_type="financial",
        required_data_categories=["financial"],
    )
    store = InsightStore(insights=[
        Insight(
...
def test_inspect_context():
    ...
    context = DecisionContext(
        run_id="test-123",
        question="Test question",
        problem_analysis={"decision_type": "financial", "required_data_categories": ["financial"]},
        relevant_data_categories=["financial"],
        relevant_storage_tiers=["tier_1", "tier_2"],
        relevant_ontology_concepts=[
            {"concept_id": "revenue", "name": "Revenue", "type": "metric", "required": True},
            {"concept_id": "expense", "name": "Expense", "type": "metric", "required": True},
        ],
        relevant_insights=[
            InsightEvidence(
                insight_id="ins-1",
        ...
        judge_summary={"confidence_level": "low", "human_review_required": ["Resolve contradictions"]},
...
def test_builder_creates_human_review_from_contradiction(tmp_path):
    ...
    assert any("contradiction" in item.lower() or "conflict" in item.lower() for item in context.human_review_items)