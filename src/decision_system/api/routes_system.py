...
def _count_workspaces(settings) -> int:
    ...
    data_dir = (
        Path(settings.workspace_db_path).parent
        if hasattr(settings, "workspace_db_path")
        else get_data_root()
    )
...
def system_status() -> dict:
    ...
    demo_data_available = (
        Path("demo/sample-data").is_dir() or Path("company_data/manifest.json").exists()
    )
    ...
    if not doc_parsing_available:
        warnings.append(
            "Document parsing dependencies not installed — PDF/DOCX/XLSX may not be readable."
        )
